...
def run(ctx: PipelineContext) -> None:
    ...
    for cohort in cohorts_to_process:
        # S07b outputs: {cohort}_drugs_clean_full_data.parquet
        source = source_dir / f"{cohort}_drugs_clean_full_data.parquet"

        if not source.exists():
            raise FileNotFoundError(
                f"Missing LLM-cleaned drug dataset for cohort {cohort}. "
                f"Expected: {source}\n"
                f"Please run S07b first."
            )
        ...
        # If basename exists but medicinal_product_llm_clean doesn't, use basename
        if "medicinal_product_llm_clean" not in df.columns:
            if "basename" in df.columns:
                df = df.rename({"basename": "medicinal_product_llm_clean"})
                logger.info(f"[Local] Using 'basename' column as 'medicinal_product_llm_clean'")
            else:
                raise ValueError(
                    f"[Local] Required column 'medicinal_product_llm_clean' or 'basename' not found in {source}. "
        ...
        if filtered_missing:
            ...
            for name in filtered_missing:
                ...
                if result and result.rxcui:
                    enriched_list.append((name, result))
        ...
        # Save enriched data
        output_file = output_dir / f"{cohort}_drugs_enriched_final_full_data.parquet"
