...
"""Stage S08 Direct – Enrich drug identifiers using Direct NCBI API Key Connection.

New Logic (v4):
1. Direct connection using NCBI API Key (no proxy needed)
2. Sequential fallback: RxNav → PubChem → RxNav ingredients
            ...
            async with RATE_LIMIT_SEMAPHORE:
                async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=30)) as resp:
                    ...
                    if throttling_status in ['red', 'black']:
                        logger.warning(f"[S08-Direct] PubChem throttling RED/BLACK for {name}, sleeping 10s")
                        await asyncio.sleep(10)
                        continue
                    elif throttling_status == 'yellow':
                        logger.warning(f"[S08-Direct] PubChem throttling YELLOW for {name}, sleeping 3s")
                        await asyncio.sleep(3)

                    if resp.status == 404:
                        logger.info(f"[S08-Direct] PubChem not found (404) for: {name}")
                        return None
                    elif resp.status == 429:  # Too many requests
                        if attempt < 2:  # Don't sleep on last attempt
                            logger.warning("[S08-Direct] PubChem rate limited (429), sleeping 2s for: %s", name)
                            await asyncio.sleep(2)
                            continue
                        return None
                    elif resp.status == 503:  # Server Busy
                        if attempt < 2:  # Don't sleep on last attempt
                            logger.warning(f"[S08-Direct] PubChem server busy (503), sleeping 5s for: {name}")
                            await asyncio.sleep(5)
                            continue
                        return None
                    elif resp.status >= 500:  # Other server errors
                        logger.warning(f"[S08-Direct] PubChem server error {resp.status} for {name}")
                        return None
                    ...
                    if resp.status == 200:
                        ...
                        if "ServerBusy" in response_text or "too many requests" in response_text.lower():
                            if attempt < 2:
                                logger.warning(f"[S08-Direct] PubChem ServerBusy detected, sleeping 5s for: {name}")
                                await asyncio.sleep(5)
                                continue
                            return None
                    ...
                    logger.info(f"[S08-Direct] PubChem raw response for '{name}': {data}")
                    ...
                    if properties and len(properties) > 0:
                        ...
                        if title:
                            ...
                            logger.info(f"[S08-Direct] PubChem extracted title: '{title}' for input '{name}'")
                            return title
                        else:
                            logger.debug(f"[S08-Direct] PubChem found property but no Title for {name}")
                    else:
                        logger.debug(f"[S08-Direct] PubChem found no properties for {name}")
                    return None

        except aiohttp.ClientError as e:
            if attempt < 2:
                logger.info(f"[S08-Direct] PubChem client error for {name} (attempt {attempt + 1}): {e}")
                continue
            logger.warning(f"[S08-Direct] PubChem failed for {name} after 3 attempts: {e}")
            return None
        except asyncio.TimeoutError:
            if attempt < 2:
                logger.info(f"[S08-Direct] PubChem timeout for {name} (attempt {attempt + 1})")
                continue
            logger.warning(f"[S08-Direct] PubChem timeout for {name} after 3 attempts")
            return None
        except Exception as e:
            logger.warning(f"[S08-Direct] PubChem unexpected error for {name}: {e}")
            return None
...
async def _enrich_names_direct(
    ...
    *,
    ...
    Sequential fallback: RxNav → PubChem → RxNav ingredients
    """
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout, headers=headers) as session:
        ...
        async def handle(name: str) -> None:
            try:
                ...
                logger.debug(f"[S08-Direct] Processing '{name}' - Step 1: RxNav lookup...")
                max_retries = 2
                for attempt in range(max_retries):
                    try:
                        logger.info(f"[S08-Direct] 🔍 '{name}' - RxNav attempt {attempt+1}/{max_retries}")
                        async with RATE_LIMIT_SEMAPHORE:
                            candidates = await client.lookup_exact(name)
                            logger.info(f"[S08-Direct] 📋 '{name}' - RxNav attempt {attempt+1} returned {len(candidates) if candidates else 0} candidates")

                            if candidates:
                                rxcui = candidates[0]
                                source = "rxnav_exact"
                                logger.info(f"[S08-Direct] ✅ RxNav SUCCESS: '{name}' → RxCUI {rxcui} (attempt {attempt+1})")
                                logger.info(f"[S08-Direct] 🔗 RxNav Link: https://rxnav.nlm.nih.gov/REST/rxcui/{rxcui}/allinfo.json")
                                break  # Success, exit retry loop
                            else:
                                logger.info(f"[S08-Direct] ❌ '{name}' - RxNav attempt {attempt+1} returned no candidates (404)")
                                if attempt < max_retries - 1:
                                    logger.info(f"[S08-Direct] 🔄 '{name}' - Retrying RxNav in 1 seconds...")
                                    await asyncio.sleep(1)
                                else:
                                    logger.warning(f"[S08-Direct] ❌ RxNav failed: '{name}' not found after {max_retries} attempts")
                                    break  # No more retries
                    except Exception as e:
                        if attempt < max_retries - 1:
                            logger.warning(f"[S08-Direct] RxNav attempt {attempt+1} failed for '{name}': {e}, retrying in 2s...")
                            await asyncio.sleep(2)
                        else:
                            logger.warning(f"[S08-Direct] RxNav failed after {max_retries} attempts for '{name}': {e}")
                            break

                # Add small delay between drugs to avoid rate limiting
                await asyncio.sleep(0.1)  # 100ms delay

                # Step 2: PubChem Fallback (if RxNav failed)
                if not rxcui:
                    logger.info(f"[S08-Direct] 🔄 RxNav failed for '{name}', trying PubChem fallback...")
                    logger.info(f"[S08-Direct] 🌐 PubChem API URL: https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/{urllib.parse.quote(name, safe='')}/property/Title/JSON")

                    try:
                        pubchem_title = await _lookup_pubchem_title_direct(session, name)
                        logger.info(f"[S08-Direct] 📋 PubChem API Response for '{name}': '{pubchem_title}'")

                        if pubchem_title:
                            logger.info(f"[S08-Direct] ✅ PubChem SUCCESS: Found title '{pubchem_title}' for '{name}'")
                            logger.info(f"[S08-Direct] 🔗 PubChem Web Link: https://pubchem.ncbi.nlm.nih.gov/compound/{urllib.parse.quote(name, safe='')}")
                            logger.info(f"[S08-Direct] 🔄 Retrying RxNav with PubChem title: '{pubchem_title}'")
                            logger.info(f"[S08-Direct] 🌐 RxNav Retry URL: https://rxnav.nlm.nih.gov/REST/rxcui.json?name={urllib.parse.quote(pubchem_title, safe='')}")
                            ...
                            for attempt in range(3):
                                try:
                                    async with RATE_LIMIT_SEMAPHORE:
                                        ...
                                        logger.info(f"[S08-Direct] 📋 RxNav retry result: {len(candidates) if candidates else 0} candidates found")

                                        if candidates:
                                            rxcui = candidates[0]
                                            source = "pubchem_fallback"
                                            logger.info(f"[S08-Direct] 🎉 PUBCHEM FALLBACK SUCCESS: '{name}' → '{pubchem_title}' → RxCUI {rxcui}")
                                            logger.info(f"[S08-Direct] 🔗 RxNav Link: https://rxnav.nlm.nih.gov/REST/rxcui/{rxcui}/allinfo.json")
                                            break
                                        else:
                                            logger.info(f"[S08-Direct] ❌ RxNav found no candidates for PubChem title '{pubchem_title}'")
                                            break  # No candidates
                                except Exception as e:
                                    if attempt < 2:
                                        logger.warning(f"[S08-Direct] PubChem RxNav retry {attempt+1} failed for '{name}': {e}")
                                        await asyncio.sleep(1)
                                    else:
                                        logger.warning(f"[S08-Direct] PubChem RxNav retry failed for '{name}': {e}")
                                        break
                        else:
                            logger.info(f"[S08-Direct] ❌ PubChem found no title for '{name}' (404 Not Found)")
                    except Exception as e:
                        logger.warning(f"[S08-Direct] PubChem lookup failed for '{name}': {e}")
                ...
                if rxcui and rxcui.strip():
                    try:
                        ...
                        logger.warning(f"[S08-Direct] Failed to get ingredients for RxCUI {rxcui} ({name}): {e}")
                        results[name] = EnrichmentResult(
                            rxcui=rxcui,
                            rxcui_tty=None,
                            ingredients=[],
                ...
                logger.debug(f"[S08-Direct] '{name}' - Final result: rxcui={rxcui}, source={source}")

                # Check if results dict was updated
                if name in results:
                    result_obj = results[name]
                    logger.debug(f"[S08-Direct] '{name}' - Results dict entry: rxcui={result_obj.rxcui}, source={result_obj.source}")
                else:
                    logger.warning(f"[S08-Direct] '{name}' - NOT FOUND IN RESULTS DICT!")

            except Exception as exc:
                logger.warning(f"[S08-Direct] Enrichment failed for '{name}': {str(exc)} (type: {type(exc).__name__})")
                logger.debug(f"[S08-Direct] Full exception for '{name}':", exc_info=True)
                results[name] = EnrichmentResult(
                    rxcui=None,
                    rxcui_tty=None,
                    ingredients=[],
                ...
                logger.debug(f"[S08-Direct] Set error result for '{name}'")

        # Process all names
        results = {}  # Initialize empty results dict
        logger.debug(f"[S08-Direct] Initialized results dict: {len(results)} entries")
        ...
        for fut in asyncio.as_completed(tasks):
            ...
            if completed % 50 == 0:  # Every 50 drugs for very frequent updates
                ...
                if completed % 1000 == 0:
                    logger.info(f"[S08-Direct] Progress: {completed}/{len(names)} drugs processed ({success_rate:.1f}% success rate)")
            ...
            if completed % 2000 == 0 and completed > 0:  # Major milestone every 2000 drugs
                ...
                logger.info(f"[S08-Direct] MILESTONE: {completed}/{len(names)} completed - {current_successful} successful so far")
                logger.info(f"[S08-Direct] Estimated final success: ~{estimated_total} drugs ({success_rate:.1f}% coverage)")

        pbar.close()

        # Final results summary
        logger.info(f"[S08-Direct] Enrichment session completed: {len(results)} results collected")

        # Debug: Check results before update
        results_with_rxcui = sum(1 for r in results.values() if r and r.rxcui and str(r.rxcui).strip())
        logger.info(f"[S08-Direct] Results dict: {len(results)} entries total, {results_with_rxcui} with valid RxCUI")
        ...
        if successful_results:
            logger.info("[S08-Direct] Sample successful enrichments:")
        ...
        logger.info(f"[S08-Direct] Final cache: {len(cache)} entries total, {total_with_rxcui} with valid RxCUI")

        return cache


def run(ctx: PipelineContext) -> None:
    """Run Stage S08 Direct: Drug identifier enrichment using Direct NCBI API."""
    ...
    if force_enrichment:
        logger.info("[S08-Direct] FORCE mode: Clearing cache, will re-run all enrichment")
        global_cache.clear()
    ...
    if target_cohort:
        ...
        logger.info(f"[S08-Direct] Processing selected cohort: {target_cohort}")
    else:
        cohorts_to_process = ("pediatric", "adult")
        logger.info("[S08-Direct] Processing all cohorts: pediatric, adult")
    ...
    for cohort in cohorts_to_process:
        ...
        logger.debug(f"[S08-Direct] Loaded DF shape: {df.shape}, columns: {list(df.columns)}")
        ...
        for col in legacy_cols:
            if col in df.columns:
                df = df.drop(col)
                logger.debug(f"[S08-Direct] Dropped legacy column: {col}")

        # Validate required columns
        if "medicinal_product_llm_clean" not in df.columns:
            raise ValueError(f"[S08-Direct] Required column 'medicinal_product_llm_clean' not found in {source}")

        logger.debug(f"[S08-Direct] After cleaning: shape {df.shape}, columns: {list(df.columns)}")
        ...
        logger.info(f"[S08-Direct] Cohort {cohort}: {len(unique_names)} unique drug names to process (sorted by frequency desc)")
        top_5 = [f"{name}({freq})" for name, freq in zip(sorted_names_df["name"].head(5), sorted_names_df["frequency"].head(5))]
        logger.info(f"[S08-Direct] Top 5 most frequent drugs: {top_5}")

        # Debug: Sample data
        logger.debug(f"[S08-Direct] Cohort {cohort}: Sample unique_names[:5]: {unique_names[:5]}")
        if global_cache:
            cache_keys = list(global_cache.keys())[:5]
            logger.debug(f"[S08-Direct] Cohort {cohort}: Sample cache keys[:5]: {cache_keys}")
        ...
        for name in missing:
            ...
            if is_obvious_non_drug:
                logger.debug(f"[S08-Direct] Skipping obvious non-drug: '{name}'")
                skipped_non_drugs += 1
        ...
        logger.info(f"[S08-Direct] Cohort {cohort}: Cache status - total unique: {len(unique_names)}, cached: {len(global_cache)}, missing: {len(missing)}")
        logger.info(f"[S08-Direct] Cohort {cohort}: Filtered out {skipped_non_drugs} obvious non-drugs, remaining to process: {len(filtered_missing)}")

        if filtered_missing:
            logger.info(f"[S08-Direct] Cohort {cohort}: {len(filtered_missing)} names need enrichment")
            logger.info(f"[S08-Direct] Cohort {cohort}: Starting enrichment function...")

            # Call enrichment function
            result_cache = asyncio.run(_enrich_names_direct(ctx, filtered_missing, global_cache, label=cohort))

            logger.info(f"[S08-Direct] Cohort {cohort}: Enrichment function returned, result_cache has {len(result_cache)} entries")
            ...
            logger.info(f"[S08-Direct] Cohort {cohort}: Enrichment completed - {enriched_count}/{len(filtered_missing)} drugs enriched")

            # CRITICAL: Debug if enriched_count is 0
            if enriched_count == 0 and len(filtered_missing) > 0:
                logger.error(f"[S08-Direct] Cohort {cohort}: ❌ CRITICAL BUG - enriched_count is 0 despite processing {len(missing)} drugs!")
                logger.error(f"[S08-Direct] Cohort {cohort}: result_cache has {len(result_cache)} entries")
                logger.error(f"[S08-Direct] Cohort {cohort}: global_cache has {len(global_cache)} entries")

                # Check sample of what went wrong
                sample_missing = filtered_missing[:5]
                logger.error(f"[S08-Direct] Cohort {cohort}: Checking first 5 missing drugs:")
                for name in sample_missing:
                    ...
                    logger.error(f"[S08-Direct]   '{name}': in_result={in_result}, in_global={in_global}, rxcui_result={rxcui_result}, rxcui_global={rxcui_global}")

            # Log some enriched drugs for verification
            if enriched_list:
                logger.info(f"[S08-Direct] Sample enriched drugs:")
                for name, result in enriched_list[:5]:
                    logger.info(f"  ✓ '{name}' → RxCUI {result.rxcui} ({result.source})")
            else:
                logger.warning(f"[S08-Direct] No drugs were successfully enriched!")
                # Check why - sample some results
                sample_results = [(name, global_cache.get(name)) for name in missing[:10]]
                logger.info(f"[S08-Direct] Sample results analysis:")
            ...
            if enriched_count > 0:
                ...
                for name in sample_names:
                    ...
                    if result and result.rxcui:
                        logger.info(f"[S08-Direct] Sample result: '{name}' → RxCUI {result.rxcui} ({result.source})")
        else:
            logger.info(f"[S08-Direct] Cohort {cohort}: All {len(unique_names)} drugs already in cache")
        ...
        logger.info(f"[S08-Direct] Cohort {cohort}: Final cache status - {len(global_cache)} total entries, {total_in_cache} with RxCUI")

        # Build mapping DataFrame from cache
        logger.debug(f"[S08-Direct] Building map DataFrame for {len(unique_names)} unique names")
        map_rows = []
        ...
        logger.info(f"[S08-Direct] Cohort {cohort}: Map DataFrame created - {found_in_cache}/{len(unique_names)} drugs have RxCUI")

        # Create output DataFrame
        map_df = pl.DataFrame(map_rows)
        logger.debug(f"[S08-Direct] Map DF shape: {map_df.shape}, columns: {list(map_df.columns)}")
        ...
        for col in required_map_cols:
            if col not in map_df.columns:
                raise ValueError(f"[S08-Direct] Required column '{col}' missing from map DataFrame")

        # Debug: Log column names before join
        logger.debug(f"[S08-Direct] Original DF columns: {list(df.columns)}")
        logger.debug(f"[S08-Direct] Map DF columns: {list(map_df.columns)}")
        ...
        logger.debug(f"[S08-Direct] After join columns: {list(enriched_df.columns)}")

        # Handle column rename safely (avoid duplicate column names)
        logger.debug(f"[S08-Direct] Before column handling: {list(enriched_df.columns)}")

        # Check for medicinal_product column conflicts
        if "medicinal_product" in enriched_df.columns:
            logger.warning(f"[S08-Direct] Column 'medicinal_product' already exists, dropping it to avoid duplicate error")
            enriched_df = enriched_df.drop("medicinal_product")
            logger.debug(f"[S08-Direct] After dropping medicinal_product: {list(enriched_df.columns)}")

        # Verify medicinal_product_llm_clean exists before rename
        if "medicinal_product_llm_clean" not in enriched_df.columns:
            raise ValueError(f"[S08-Direct] Column 'medicinal_product_llm_clean' not found for rename. Available columns: {list(enriched_df.columns)}")
        ...
        logger.debug(f"[S08-Direct] After rename: {list(enriched_df.columns)}")

        # Final validation
        if "medicinal_product" not in enriched_df.columns:
            raise ValueError(f"[S08-Direct] Rename failed - 'medicinal_product' column not found after rename")

        logger.info(f"[S08-Direct] Column rename completed successfully")

        # Debug: Log final columns
        logger.debug(f"[S08-Direct] Final columns: {list(enriched_df.columns)}")

        # Save enriched data
        output_file = output_dir / f"{cohort}_drugs_enriched.parquet"
        enriched_df.write_parquet(output_file)
        logger.info(f"[S08-Direct] Saved {len(enriched_df)} enriched records to {output_file}")
        ...
        logger.info(f"[S08-Direct] Cohort {cohort}: {coverage:.1f}% coverage ({mapped_rows}/{total_drugs} drugs enriched)")
        logger.info(f"[S08-Direct] Cohort {cohort}: RxNav direct: {rxnav_exact}, PubChem fallback: {pubchem_fallback}, Not found: {not_found}")

        # Quality assessment
        if coverage >= 90:
            logger.success("[S08-Direct] Cohort %s: 🌟 EXCELLENT coverage (%.1f%%) - Production ready!", cohort, coverage)
        elif coverage >= 80:
            logger.success("[S08-Direct] Cohort %s: ✅ GOOD coverage (%.1f%%) - Ready for production!", cohort, coverage)
        elif coverage >= 70:
            logger.warning("[S08-Direct] Cohort %s: ⚠️  MODERATE coverage (%.1f%%) - Acceptable for research", cohort, coverage)
        else:
            logger.error("[S08-Direct] Cohort %s: ❌ LOW coverage (%.1f%%) - May need data quality improvements", cohort, coverage)
    ...
    logger.info("[S08-Direct] 📈 OVERALL SUMMARY:")
    logger.info("[S08-Direct] Total unique drugs processed: %d", total_drugs_all)
    logger.info("[S08-Direct] Total drugs enriched: %d", total_enriched_all)
    logger.info("[S08-Direct] Overall coverage: %.2f%%", overall_coverage)

    write_manifest(
        ctx,
        "s07_enrich_drug_identifiers_direct",
        {
            "stage": "s07_enrich_drug_identifiers_direct",
            "version": "1.0",
    ...
    if overall_coverage >= 80:
        logger.success("[S08-Direct] 🎉 PIPELINE SUCCESS: %.1f%% overall coverage - Production ready!", overall_coverage)
    else:
        logger.warning("[S08-Direct] ⚠️  PIPELINE COMPLETE: %.1f%% overall coverage - Monitor and improve data quality", overall_coverage)
